...
public class NimbleNetApi {
    ...
    public static func initialize(config: NimbleNetConfig, assetsJson: [[String: Any]]? = nil)
        -> NimbleNetResult<Void>
    {
        var config = config
        ...
        do {
            ...
            if let jsonString = String(data: jsonData, encoding: .utf8) {
                if config.online {
                    res =
                        nimbleNetController.initialize_nimblenet_controller(
                            jsonString,
                            assetsJson: nil
                        )! as NSDictionary
                    ...
                    if assetsJson == nil {
                        return populateNimbleNetResultWithError(
                            errorCode: 5000,
                            errorMessage:
                                "assetsJson cannot be nil in case online flag is set to false."
                        )
                    }
                    ...
                    do {
                        try modifiedAssetsJson = FileIO.shared.processModules(
                            assetsJson: &modifiedAssetsJson!
                        )
                    ...
                    let modifiedAssetsJsonData = try JSONSerialization.data(
                        withJSONObject: modifiedAssetsJson,
                        options: []
                    )
                    guard
                        let modifiedAssetsJsonString = String(
                            data: modifiedAssetsJsonData,
                            encoding: .utf8
                        )
                    else {
                        return populateNimbleNetResultWithError(
                            errorCode: 5000,
                            errorMessage: "Failed to convert modified assetsJson to String."
                        )
                    }
                    ...
                    res =
                        nimbleNetController.initialize_nimblenet_controller(
                            jsonString,
                            assetsJson: modifiedAssetsJsonString
                        )! as NSDictionary
                    return NimbleNetResult<Void>(data: NSDictionary(dictionary: res))
                }
            } else {
                return populateNimbleNetResultWithError(
                    errorCode: 5000,
                    errorMessage: "invalid nimblenet config"
                )
            }
        } catch {
            return populateNimbleNetResultWithError(
                errorCode: 5000,
                errorMessage: "exception: initNimbleNet \(error)"
            )
        }
    }

    public static func initialize(config: String, assetsJson: [[String: Any]]?) -> NimbleNetResult<
        Void
    > {
        if let jsonData = config.data(using: .utf8) {
            ...
            return populateNimbleNetResultWithError(
                errorCode: 5000,
                errorMessage: "invalid nimblenet config json"
            )
        }
    }

    public static func addEvent(events: [String: Any], eventType: String) -> NimbleNetResult<
        UserEventdata
    > {
        do {
            ...
            if let jsonString = String(data: jsonData, encoding: .utf8) {
                let res = nimbleNetController.add_event_controller(
                    jsonString,
                    eventType: eventType
                )!
                ...
                return populateNimbleNetResultWithError(
                    errorCode: 5000,
                    errorMessage: "invalid events"
                )
            }
        } catch {
            return populateNimbleNetResultWithError(
                errorCode: 5000,
                errorMessage: "exception: addEvent \(error)"
            )
        }
    }

    public static func addEvent(eventString: String, eventType: String) -> NimbleNetResult<
        UserEventdata
    > {
    ...
    public static func isReady() -> NimbleNetResult<Unit> {
        do {
            ...
            return populateNimbleNetResultWithError(
                errorCode: 5000,
                errorMessage: "exception: isReady \(error)"
            )
        }
    }

    public static func runMethod(methodName: String, inputs: [String: NimbleNetTensor])
        -> NimbleNetResult<NimbleNetOutput>
    {
        do {
        ...
        var res = nimbleNetController.run_task_controller(
            methodName,
            modelInputsWithShape: inputDict
        )!
                    ...
                    if var data = tensorDetails["data"] {
                        if let wrapper = data as? ProtoObjectWrapper {
                            do {
                                ...
                                return populateNimbleNetResultWithError(
                                    errorCode: ProtoError.failure.rawValue,
                                    errorMessage: "proto decoding failed \(error)"
                                )
                            }
    ...
    private static func populateNimbleNetResultWithError<T>(errorCode: Int, errorMessage: String)
        -> NimbleNetResult<T>
    {
...
func verifyUserInputs(inputs: [String: NimbleNetTensor]) throws {
    for (_, modelInput) in inputs {
        ...
        default:
            throw DataTypeMismatchError.unsupportedDataType(
