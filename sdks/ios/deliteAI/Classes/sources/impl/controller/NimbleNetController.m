...
- (NSDictionary*)initialize_nimblenet_controller:(NSString*)configJson
                                      assetsJson:(NSString*)assetsJson {
    ...
    NSData* directorySizeJsonData = [NSJSONSerialization dataWithJSONObject:sizes
                                                                    options:0
    ...
    if (nimbleSdkDirectory != nil) {
        ...
        if (assetsJson != nil) {
            NimbleNetStatus* status =
                load_modules([assetsJson UTF8String], [nimbleSdkDirectory UTF8String]);
            if (status != nil) {
                NSDictionary* res = @{
                    @"status" : @false,
                    @"data" : [NSNull null],
        ...
        NimbleNetStatus* status =
            initialize_nimblenet([configJson UTF8String], [nimbleSdkDirectory UTF8String]);
        ...
        if (status == NULL) {
            if (!directorySizeError) {
                NSString* jsonString = [[NSString alloc] initWithData:directorySizeJsonData
...
NSDictionary* convertCUserEventsDataToNSDictionary(CUserEventsData* data) {
    ...
    NSString* eventJsonString =
        data->eventJsonString ? [NSString stringWithUTF8String:data->eventJsonString] : nil;
...
- (NSDictionary*)add_event_controller:(NSString*)eventMapJsonString eventType:(NSString*)eventType {
    ...
    NimbleNetStatus* status =
        add_event([eventMapJsonString UTF8String], [eventType UTF8String], &cUserEventsData);
...
- (NSString*)parseCrashReportwithKeyword:(NSString*)crashLog keyword:(NSString*)keyword {
    ...
    for (NSString* line in lines) {
        if ([line containsString:@"Crashed Thread:"]) {
            NSString* threadNumString =
                [[line componentsSeparatedByString:@"Crashed Thread:"].lastObject
    ...
    for (NSString* line in lines) {
        if ([line containsString:threadPrefix]) {
            isCrashedThread = YES;
                   ...
                   ([line hasPrefix:@"Thread"] || [line hasPrefix:@"Binary Images:"])) {
            isCrashedThread = NO;
        }
    ...
    if (result.length == 0 ||
        [result isEqualToString:[NSString stringWithFormat:@"%@\n", metadata]]) {
        return [NSString
            stringWithFormat:@"%@\nNo lines with the keyword '%@' found in the crashed thread.",
                             metadata, keyword];
    }
...
- (NSString*)parseAndLogCrashLog:(NSString*)log {
    ...
    for (NSString* line in lines) {
        if ([line hasPrefix:@"Date/Time:"]) {
            dateTime = [[line substringFromIndex:[@"Date/Time:" length]]
                stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];

            // Remove the "+00" time zone part
            NSRange timeZoneRange = [dateTime rangeOfString:@" +0000"];
            if (timeZoneRange.location != NSNotFound) {
                dateTime = [dateTime stringByReplacingCharactersInRange:timeZoneRange
                                                             withString:@""];
            }
            ...
            exceptionType = [[line substringFromIndex:[@"Exception Type:" length]]
                stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
        } else if ([line hasPrefix:@"Exception Codes:"]) {
            exceptionCode = [[line substringFromIndex:[@"Exception Codes:" length]]
                stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
            ...
            NSRegularExpression* regex =
                [NSRegularExpression regularExpressionWithPattern:@"^(\\d+|0x[0-9A-Fa-f]+)"
                                                          options:0
                                                            error:nil];
            ...
            if (range.location != NSNotFound) {
                [backtrace
                    addObject:[line stringByTrimmingCharactersInSet:[NSCharacterSet
    ...
    NSString* output =
        [NSString stringWithFormat:
                      @"METRICS::: %@ ::: crash ::: "
                      @"{\"errorCode\":\"%@\",\"backtrace\":\"%@\",\"signalCode\":\"%@\"}",
...
- (void)write_run_method_metric_controller:(const char*)modelId
                          totalTimeInUSecs:(int64_t)totalTimeInUSecs {
