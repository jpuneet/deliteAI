...
NSDictionary* convertCTensorsToNSDictionary(NimbleNetStatus* status, CTensors ctensors,
            ...
            if (dataLength == 0) {
                switch (tensor.dataType) {
                    ...
                    case JSON: {
                        ...
                        NSDictionary* dict =
                            convertvoidPointertoJsonObject(jsonIterator, json_alloc);
                    ...
                    case FE_OBJ: {
                        ...
                        switch (proto->type) {
                            case IOS_PROTO_OBJECT: {
                                ProtoObjectWrapper* castedObject =
                                    (__bridge ProtoObjectWrapper*)proto->obj;
                                data = castedObject;
                                break;
                            }
                            case IOS_ANY_OBJECT: {
                                ProtoAnyWrapper* castedObject =
                                    (__bridge ProtoAnyWrapper*)proto->obj;
                                data = castedObject;
                                break;
                            }
                            case IOS_MAP: {
                ...
                switch (tensor.dataType) {
                    case STRING: {
                        ...
                        for (int j = 0; j < totalArrayLength; j++) {
                            if (castedArrayData[j] != NULL) {
                                NSString* string =
                                    [NSString stringWithUTF8String:castedArrayData[j]];
                    ...
                    case JSON_ARRAY: {
                        void* jsonIterator = create_json_iterator(tensor.data, json_alloc);
                        NSArray* jsonArray =
                            convertvoidPointertoJSONArray(jsonIterator, json_alloc);
    ...
    resultDict = @{
        ...
        @"data" : status != NULL
            ? [NSNull null]
            : @{@"outputs" : outputsTensorDict, @"size" : @(ctensors.numTensors)},
...
NSMutableArray* convertvoidPointertoJSONArray(void* jsonIterator, void* json_alloc) {
    ...
    while (true) {
        ...
        switch (nextElement->dataType) {
            ...
            case JSON: {
                NSDictionary* nestedObject =
                    convertvoidPointertoJsonObject(nextElement->value.obj, json_alloc);
